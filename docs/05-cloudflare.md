...
Finalization performs one R2 read per accepted file, plus database operations. The checked-in Worker configuration sets `[limits] subrequests = 25000` so a Workers Paid deployment with a custom subrequest allowance can finalize the existing 20,000-file maximum. Workers Free remains limited by Cloudflare's 1,000 internal-service subrequests per invocation and therefore cannot finalize a full 20,000-file direct upload; use a paid/custom allowance or keep manifests below the applicable platform limit.
