---
title: How OpenDrop Works
description: The upload, validation, storage, rendering, versioning, visibility, and annotation model.
---
...
3. Validation checks file count, total size, per-file size, text line counts, unsafe paths, duplicates, and root `index.html`.
...
`/{namespace}/{slug}` resolves to the latest version. `?version={versionId}` pins a specific immutable version. Legacy path URLs such as `/{namespace}/{slug}/versions/{versionId}` redirect to the query form.
...
Share URLs render the full-screen review room:
...
The review room loads the uploaded site inside a sandboxed iframe and keeps comments, visibility controls, and version switching in floating UI around the preview. Relative assets are served from object storage under the same share URL.
