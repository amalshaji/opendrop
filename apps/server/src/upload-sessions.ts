...
export function registerUploadSessionRoutes(
  ...
  app.post("/api/uploads/sessions/:sessionId/finalize", async (c) => {
    ...
    if (session.status === "completed") {
      return completedSessionResponse(c, repo, session);
    }
    if (session.status === "failed") {
      return c.json({ error: session.failureReason ?? "Upload session failed." }, 409);
    }
    if (session.status === "finalizing") {
      return finalizingSessionResponse(c, repo, storage, session);
    }
    ...
    if (claim.outcome === "completed") {
      return completedSessionResponse(c, repo, claim.session);
    }
    if (claim.outcome === "failed") {
      return c.json({ error: claim.session.failureReason ?? "Upload session failed." }, 409);
    }
    if (claim.outcome === "in_progress") {
      return finalizingSessionResponse(c, repo, storage, claim.session);
    }
    ...
    const deployment = await repo.completeUploadSession({
    ...
    return c.json(sessionPublishResponse({ ...session, status: "completed" }, deployment));
  });
}
...
async function failSessionAndClean(
  ...
  expectedStatus: "pending" | "finalizing",
  reason: string
): Promise<void> {
  const failed = await repo.failUploadSession({
    sessionId: session.id,
    ownerUserId: session.ownerUserId,
    expectedStatus,
    reason
...
async function finalizingSessionResponse(
  c: OpenDropContext,
  repo: OpenDropRepository,
  storage: ArtifactStorage,
  session: UploadSessionRecord
...
async function completedSessionResponse(
  ...
  const deployment = await repo.getDeploymentVersion(session.namespace, session.slug, session.versionId);
  if (
    !deployment ||
    deployment.version.createdByUserId !== session.ownerUserId ||
    deployment.version.manifestHash !== session.manifestHash
  ) {
    return c.json({ error: "Completed upload session has no matching deployment." }, 500);
  }
  return c.json(sessionPublishResponse(session, deployment));
}
